...
from pushworld.model import CustomCNN, CustomPolicy, train_ppo, train_dqn
...
wandb.init(project="test_", config={**config_train, **config},name="check_dqn")
...
def test_model(model):
    ...
    fig.savefig(path_to_rep + "python3/fotos/dqn_test_ac.png")
    plt.close(fig)
    fig, ax = plt.subplots()
    #plt.figure(figsize=(8, 5))
    ...
    fig.savefig(path_to_rep + "python3/fotos/dqn_train_ac.png")
...
model = train_dqn(menv, combined_callback, **config_train)
